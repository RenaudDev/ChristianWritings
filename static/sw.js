...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`;
  ...
  // static/sw.ts
  var CACHE_PREFIX = "workbox-renaud-coding";
  var CACHE_OFFLINE_NAME = `${CACHE_PREFIX}-offline`;
  var CACHE_SCRIPT_NAME = `${CACHE_PREFIX}-scripts`;
  var CACHE_STYLES_NAME = `${CACHE_PREFIX}-styles`;
  var CACHE_DOCUMENTS_NAME = `${CACHE_PREFIX}-documents`;
  var CACHE_FONTS_NAME = `${CACHE_PREFIX}-fonts`;
  var CACHE_IMAGES_NAME = `${CACHE_PREFIX}-images`;
  precacheAndRoute(self.__WB_MANIFEST);
  var stylesScriptsExpirationPlugin = new ExpirationPlugin({
    maxEntries: 10,
    maxAgeSeconds: 15 * 24 * 60 * 60,
    purgeOnQuotaError: true
  });
  var fontsExpirationPlugin = new ExpirationPlugin({
    maxEntries: 5,
    maxAgeSeconds: 180 * 24 * 60 * 60
  });
  var imagesExpirationPlugin = new ExpirationPlugin({
    maxEntries: 100,
    maxAgeSeconds: 60 * 24 * 60 * 60
  });
  var documentExpirationPlugin = new ExpirationPlugin({
    maxEntries: 50,
    maxAgeSeconds: 60 * 24 * 60 * 60,
    purgeOnQuotaError: true
  });
  var registerCacheFirstRouteUsing = (destination, cacheName, expirationPlugin) => registerRoute(
